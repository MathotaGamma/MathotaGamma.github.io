/*
  ...
  v6.5: authを本番環境向けに修正。
    - silent=falseの場合、promptの初期値を'select_account'にし、アカウント選択画面を確実に表示するようにした。
      (呼び出し側で明示的にnullを渡せば従来通り抑制可能)
    - window.open後にpopup.focus()を呼び、ポップアップが前面に表示されるようにした。
    - closedCheckTimer / blockCheckTimer / pollTimerの3系統だったタイマーを1本のpollTimerに統合し、
      「認証成功→ポップアップ自動close」が「ユーザーによるキャンセル」と誤判定されるレースコンディションを解消。
      (popup.closedを検知しても即エラーにはせず、GRACE_MS分だけoauth_resultの書き込みを待ってから確定する)
    - popup.closedがundefined(一部のポップアップブロッカー特有の挙動)になるケースも検出するようにした。
  v6.6: request()がJSON.parseを行わず常に生テキスト(または204時の{ok:true})を返す仕様に変更されていたが、
        呼び出し側の多くが依然としてres.filesやres.parentsなどオブジェクトとして扱っており、
        実行時エラー(文字列に対するプロパティアクセス)になっていたバグを修正。
        共通ヘルパー _toJSON() を追加し、JSONオブジェクトとして扱う必要のある呼び出し側
        (getAbout, createFile, getFileId, getPath, getParentId, existCheck, getFileInfo,
         listFiles, removeFile, saveFile, #miniGetStructure)でこれを介してパースするよう統一。
        ※getFile({alt:'media'})はファイル本体(JSONとは限らない)を返すため、意図的にパースしない。
  ※本番運用にあたっては、redirectUriのページが「このアプリと同一オリジン」であり、
    認証結果をlocalStorage(key: 'oauth_result')に書き込んだ上でwindow.close()すること、
    という前提は変わりません(localStorageはオリジン単位で分離されるため)。
*/
class DriveAPIManager {
  ...
  // v6.6: request()は常に生テキスト(または204時のオブジェクト)を返すため、
  // JSONオブジェクトとして扱いたい呼び出し側はここを通して安全にパースする。
  // - すでにオブジェクト(204の{ok:true}など)ならそのまま返す
  // - 文字列ならJSON.parseを試み、失敗したら元の文字列をそのまま返す
  _toJSON(raw) {
        ...
        .setCallback(async (data) => { // async コールバックに変更
          ...
          if (data.action === window.google.picker.Action.PICKED) {
            ...
            resolve({
              id: doc.id,
              name: doc.name,
              path: returnPath, // パス（例: "folderA/subfolderB/file.txt"）を返り値に追加
  ...
  // v6.5: silent=falseの既定でアカウント選択画面を確実に表示するよう prompt の初期値を 'select_account' に変更。
  // 明示的に auth(false, null) を呼べば従来通り prompt パラメータなしで実行できる。
  auth(silent = false, prompt = 'select_account') {
    ...
    const popup = window.open(targetUrl, popupName, 'width=500,height=600,left=100,top=100,menubar=no,toolbar=no,location=no,status=no');

    // ポップアップブロック検出...window.open が null/undefined を返す場合
    if (!popup) {
      this.progress('auth', 'popup_blocked');
      return Promise.resolve({ ok: false, error: 'popup_blocked' });
    }

    // v6.5: ウインドウを確実に前面表示する
    try {
      popup.focus();
    } catch (e) {
      // フォーカスに失敗しても致命的ではないため握りつぶす
    }

    this._authPromise = new Promise((resolve) => {
      let resolved = false;
      let closedDetectedAt = null;
      const GRACE_MS = 500; // popup.closed検知後、oauth_resultの書き込みを待つ猶予時間
      ...
      // v6.5: closedCheckTimerとpollTimerを1本に統合し、
      // 「認証成功直後にポップアップが自動closeする」ケースを誤って
      // 「ユーザーによるキャンセル」と判定してしまうレースコンディションを解消。
      const pollTimer = setInterval(() => {
        ...
        if (rawResult) {
          ...
          try {
            ...
            if (data.token) {
              ...
              finish({ ok: true, token: data.token, silent: false });
            }
          } catch (e) {
            this.progress('auth', 'fail');
        ...
        // popup.closed が undefined になる一部のブロッカーにも対応
        const isClosed = popup.closed === true || popup.closed === undefined;

        if (isClosed) {
          if (closedDetectedAt === null) {
            // 閉じたことを検知しても即エラーにはせず、猶予時間分だけ結果の書き込みを待つ
            closedDetectedAt = Date.now();
            return;
          }
          if (Date.now() - closedDetectedAt >= GRACE_MS) {
            this.progress('auth', 'popup_closed_by_user');
            finish({ ok: false, error: 'popup_closed_by_user' });
          }
        } else {
      ...
      // ポップアップブロック検出...開いた直後に closed になっている場合(遅延ブロック)
      const blockCheckTimer = setTimeout(() => {
        if (!resolved && (popup.closed === true || popup.closed === undefined)) {
          this.progress('auth', 'popup_blocked');
  ...
  async getAbout(fields) {
    const raw = await this.request('GET', 'about', {
      params: { fields }
    });
    return this._toJSON(raw);
  }
  ...
  async createFile({parentPath, parentId, name, mimeType, description=''}) {
    ...
    const raw = await this.request('POST', 'files', { body: meta });
    return this._toJSON(raw);
  }
  ...
  async getFileId({path}) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      const raw = await this.request('GET', 'files', {
        params: { q, spaces: this.spacesParam, fields: 'files(id)' }
      });
      const res = this._toJSON(raw);
  ...
  async getPath({fileId, rootCut=false}) {
    ...
    try {
      do {
        const raw = await this.request('GET', `files/${currentId}`, {
  ...
  async getParentId({path, id}) {
    ...
    const raw = await this.request('GET', `files/${targetId}`, {
  ...
  async listFiles({path, fileId}) {
    ...
    do {
      ...
      const raw = await this.request('GET', 'files', { params });
      const res = this._toJSON(raw);
  ...
  async removeFile({path, fileId}) {
    ...
    const raw = await this.request('PATCH', `files/${targetId}`, {
      body: { trashed: true }
    });
    const file = this._toJSON(raw);
  ...
  */
  async saveFile({path, fileId, data, mimeType="application/json"}) {
    ...
    if (_fileId && _fileId !== this.rootId) {
      const raw = await this.request('PATCH',
  ...
  async getFile({path, fileId}) {
    ...
    // alt: media はファイル本体(テキスト/バイナリ)を返すため、意図的にJSONパースしない
