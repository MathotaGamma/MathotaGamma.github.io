...
configure ${configureArgs}`
  ...
  step(state) {
    ...
    // バグ修正: evaluationStep()にはNaNチェックがあったが、step()(通常の学習用)には
    // 無かった。この非対称性により、学習が発散してQ値がNaNになっても
    // step()側は気づかず学習が(無意味なまま)進み続け、evalモードに入った
    // 瞬間だけevaluationStep()のNaNチェックに引っかかって例外が飛ぶ、という
    // 「evalの時だけ動かなくなる」現象が起きうる。ここで揃えて早期に検知する。
    if (actions.includes(NaN))
      this.throwError('step', '#forwardの戻り値にNaNが含まれています(学習が発散している可能性があります)。');
  ...
  // tauを指定すると、targetNetworkの重みをonlineNetworkの重みへ完全に置き換える
  // のではなく、target = tau*online + (1-tau)*target で緩やかに追従させる
  // (Polyak平均によるソフト更新)。
  // 追加理由: 1000stepごとのハード同期(tau未指定時の従来動作)は、
  // 方策が良くなった直後に同期が起きると、ブートストラップ先のQ値が
  // 一気に変わってしまい、そこから学習が崩れる("収束直後に急にスコアが
  // 落ちる")引き金になりうる。ソフト更新は毎step呼ぶことでこの急変を防ぐ。
  updateTargetNetwork(tau = null) {
    const onlineWeights = this.#onlineNetwork.getWeights();
    if (tau == null) {
      this.#targetNetwork.setWeights(onlineWeights);
      return;
    }
    if (!(tau > 0 && tau <= 1))
      this.throwError('updateTargetNetwork', 'tauは0より大きく1以下の数値である必要があります。');
    const targetWeights = this.#targetNetwork.getWeights();
    const blended = onlineWeights.map((onlineArr, i) => {
      const targetArr = targetWeights[i];
      if (onlineArr == null || targetArr == null)
        return targetArr;
      const out = new onlineArr.constructor(onlineArr.length);
      for (let j = 0; j < onlineArr.length; j++)
        out[j] = tau * onlineArr[j] + (1 - tau) * targetArr[j];
      return out;
    });
    this.#targetNetwork.setWeights(blended);
  }
