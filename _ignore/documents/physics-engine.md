...
クォータニオンは4つの実数 `(w, x, y, z)` の組で、「ある軸まわりにある角度だけ回転する」という情報を圧縮して持っている。

```
q = (w, x, y, z)
    w     : スカラー部。回転角度 θ の「cos(θ/2)」に対応する
    x,y,z : ベクトル部。回転軸の向きを sin(θ/2) でスケーリングしたもの
```

**θ とは何か:**  
θ はこのクォータニオンが表す**回転角度(ラジアン)**のこと。  
例えば Y 軸まわりに 90° 回転させたければ θ = π/2 (= 1.5708 rad) となる。  
w と x,y,z の両方に θ/2 を使っているのは、クォータニオン積の性質上そうしないと  
2回かけたときに θ ではなく 2θ の回転になってしまうため(ダブルカバー性という)。

**単位クォータニオン (回転なし・初期状態):**  
θ = 0 のとき cos(0) = 1, sin(0) = 0 となるので:
```
q_identity = (1, 0, 0, 0)
```
...
q = ( cos(θ/2),  sin(θ/2)·nx,  sin(θ/2)·ny,  sin(θ/2)·nz )
  ...
  θ   : 回転角度 [ラジアン]。これがこのクォータニオンが表す「どれだけ回転するか」
  n̂   : 回転軸(単位ベクトル。|n̂| = 1 でないといけない)
  nx,ny,nz : n̂ の各成分
```

**計算例:** Y軸まわりに90°回転
```
n̂ = (0, 1, 0),  θ = π/2 ≈ 1.5708 [rad]  (= 90°)

w = cos(π/4) ≈ 0.7071
x = sin(π/4) · 0 = 0       ← n̂ の x 成分が 0 なのでゼロ
y = sin(π/4) · 1 ≈ 0.7071  ← n̂ の y 成分が 1 なのでそのままの値
z = sin(π/4) · 0 = 0
...
**クォータニオン積 (回転の合成):**  
「まず q_A で回転、次に q_B で回転」を1つのクォータニオンで表す操作。  
ここで `pw, px, py, pz` は p のスカラー部とベクトル部の各成分、  
`qw, qx, qy, qz` は q の各成分を指す。順序が重要: `p * q ≠ q * p`(行列の乗算と同様)。
```
...
#### ベクトルをクォータニオンで回転させる方法

3Dベクトル v = (vx, vy, vz) をクォータニオン q が表す回転で動かす方法は2つある。

**方法1: クォータニオンの積として計算する(数式的に正確)**

「純虚クォータニオン」という概念を使う。  
純虚クォータニオンとはスカラー部がゼロで、ベクトル部だけを持つクォータニオンのこと。  
3Dベクトル v = (vx, vy, vz) を純虚クォータニオン `v_q = (0, vx, vy, vz)` として表現し、  
クォータニオン積で「サンドイッチ」する形で回転を適用する。

```
v_q    = (0, vx, vy, vz)   ← スカラー部を 0 にして v を純虚クォータニオンとして表現
q_inv  = (w, -x, -y, -z)   ← q の逆クォータニオン(単位クォータニオンなら共役と等しい)

v'_q = q * v_q * q_inv     ← 3回のクォータニオン積で回転を計算

回転後のベクトル v' = (v'_q.x, v'_q.y, v'_q.z)  ← 結果のベクトル部を取り出す
```

スカラー部を 0 にする理由: クォータニオン積の計算上、純粋な3D回転を表すためには  
入力もスカラー部がゼロの「純虚クォータニオン」でないといけない。  
スカラー部に何らかの値があると結果が正しい3D回転にならない。

**方法2: 回転行列に変換してから乗算する(実装で一般的)**

クォータニオン積を3回計算するより、一度 3×3 行列に変換してしまい、  
その後はベクトルとの行列積(3×3 × 3×1 = 3回の乗算)で計算する方が通常高速。  
特に同じクォータニオンで何本もベクトルを回転させる場合に有効。

クォータニオン q = (w, x, y, z) から回転行列 R を求める式:  
ここで w, x, y, z はクォータニオン q の各成分(スカラー部と3つのベクトル部)を指す。

```
R = [
  1−2(y²+z²),   2(xy−wz),    2(xz+wy)   ← R の第1行
  2(xy+wz),    1−2(x²+z²),   2(yz−wx)   ← R の第2行
  2(xz−wy),     2(yz+wx),   1−2(x²+y²)  ← R の第3行
]

  この式の w,x,y,z はクォータニオン q の4成分のこと。θ は直接は登場しないが、
  w = cos(θ/2), x = sin(θ/2)·nx ... という関係を代入展開したものが上記の式。
```

**R を使ったベクトルの回転:**
```
// ローカル座標のベクトル v_local をワールド座標に変換する
v_world = R · v_local

// 具体的な計算(行ベクトルに各行の内積):
v_world.x = R[0][0]·v_local.x + R[0][1]·v_local.y + R[0][2]·v_local.z
v_world.y = R[1][0]·v_local.x + R[1][1]·v_local.y + R[1][2]·v_local.z
v_world.z = R[2][0]·v_local.x + R[2][1]·v_local.y + R[2][2]·v_local.z
```

**R はどちら向きの変換か:**  
物体の向きを表すクォータニオン `q` から生成した R は「ローカル → ワールド」変換。  
ワールド → ローカルに変換したいときは転置行列 `Rᵀ` を使う(直交行列なので `Rᵀ = R⁻¹`)。

```
// 使用例: ローカル座標の支持点をワールド座標に変換
support_world = R · support_local + body.position

// 使用例: ワールド座標の方向 d をローカル座標に変換(支持関数に渡すため)
d_local = Rᵀ · d_world
```
...
**正規化 (毎フレーム行う):**  
積分の数値誤差でクォータニオンが「単位クォータニオン」でなくなっていく。  
単位クォータニオンでないと `|q| ≠ 1` となり、回転行列に変換したとき数値がおかしくなる。  
毎フレーム正規化することで誤差を補正する。
```
|q| = sqrt(w²+x²+y²+z²)
q_normalized = q / |q|   ← 各成分を長さで割る
```
...
#### 支持関数 (Support Function) とは何か — GJK の前提知識

GJK を理解するには「支持関数」を先に把握する必要がある。  
§2 の各形状の説明で support 関数が登場していたが、ここで改めてその意味と役割を整理する。

**支持関数の定義:**  
形状 S に対して、任意の方向ベクトル d が与えられたとき、  
「形状の内部または表面の点の中で、d 方向に最も遠い点」を返す関数。

言い換えると「d 方向に形状をどこまで伸ばしても、最もはみ出た点はどこか」を返す。

```
support_S(d) = 形状 S 上の点 p のうち、dot(p, d) が最大になる点
             = argmax_{p ∈ S} dot(p, d)
```

**各形状での support の計算例:**
```
// Sphere: 中心から d 方向に radius 分進んだ点
support_Sphere(d) = center + radius · normalize(d)

// Box: 各軸について d の符号と同じ側の頂点成分を選ぶ
support_Box(d_local) = ( sign(d.x)·hx, sign(d.y)·hy, sign(d.z)·hz )

// Polyhedron: 全頂点の中で dot(v, d) が最大の頂点を線形探索
support_Poly(d) = vertices[ argmax dot(v, d) ]
```

**何に使うのか:**  
GJK は形状の全ての点を知る必要がなく、「指定した方向の最遠点だけ」を問い合わせることで  
衝突判定を完結できる。これが GJK を様々な凸形状に対して汎用的に使える理由。
...
直感的な説明:  
A と B が接触しているとき「A のある点 a と B のある点 b が同じ位置にある (a = b)」。  
そのとき `a - b = 0` となる点が A⊖B の中に存在する = 原点が A⊖B の中にある、ということ。  
逆に A と B が離れているなら、どの `a - b` の組み合わせも 0 にはならない = 原点が A⊖B の外にある。

**問題の変換:**  
「A と B は交差しているか？」という問いが  
「A ⊖ B という1つの形状に原点が含まれるか？」という問いに変換される。  
これにより、2形状の相互判定という複雑な問題が、単一形状と点の包含判定という単純な問題になる。

**ミンコフスキー差上の支持関数:**  
A ⊖ B を明示的に全点計算するのは無駄。  
方向 d に対して A ⊖ B 上の最遠点だけを求める式が以下:
...
なぜこの式で求まるか:  
`a - b` を最大化したい(= d 方向の最遠点を求めたい)とき、  
a を d 方向に最大化しつつ b を -d 方向に最大化(= d 方向に最小化)すれば良い。  
つまり「A の d 方向の最遠点」から「B の -d 方向の最遠点」を引けば A⊖B の最遠点になる。

---

#### シンプレックスとは何か

**シンプレックス(Simplex)** とは、GJK が内部で管理する「点の集合」のこと。

GJK の目的は「A ⊖ B という形状が原点を含むか」を判定すること。  
しかし A ⊖ B の全点を調べるのは計算コストが高すぎる。  
そこで GJK は「原点を囲もうとする小さな多面体(= シンプレックス)」を少しずつ育てていく方針を取る。

シンプレックスは最大 4点で構成される。点数によって幾何学的な形状が決まる:
...
**なぜ最大4点で十分か:**  
3次元空間において「1点(原点)を完全に囲む」最小の多面体は四面体(4点)。  
4点以上あれば必ず原点を含む四面体を作れるので、4点に達した時点で判定できる。

**GJK のシンプレックスの使い方:**  
各イテレーションで「A ⊖ B の表面上の新しい点」をシンプレックスに追加し、  
「現在のシンプレックスが原点を含むか、含まないなら次はどこを探索すべきか」を判定する。  
原点に近づかない方向の点は除去して、シンプレックスを常に最小限に保つ。
