export default class CogniKeel {
  static pro(matrixA, matrixB, N, K, M) {
    return CogniKeel.Matrix.pro(matrixA, matrixB, N, K, M);
  }
...
/*
// --- 外部での実行・計測コード例 ---
import ck from "https://mathotagamma.github.io/APIs/CogniKeel/1-01-01/CogniKeel.js";
(async () => {
  const N = 1024, K = 1024, M = 1024;

  // 1. staticメソッドで外部からデータを生成して保持
  const myMatrixA = ck.randomGenerate(N, K);
  const myMatrixB = ck.randomGenerate(K, M);
  
  await new Promise(r => setTimeout(r, 50)); 
  
  // 2. 外部からデータを渡して実行時間を計測
  const start = performance.now();
  const res = ck.pro(myMatrixA, myMatrixB, N, K, M);
