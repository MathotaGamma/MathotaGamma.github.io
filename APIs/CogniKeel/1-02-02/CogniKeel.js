...
class CogniKeel extends Common {
  ...
  })
  {
    ...
    this.minReplayBufferLength = minReplayBufferLength;
...
stepCount ${stepCount == 0 ? 'null' : String(stepCount)}`
  ...
  */
  learn() {
    if (this.#replayBuffer.length < this.minReplayBufferLength)
      return {
        ok: true,
        done: false,
    ...
    const sampledReplays = this.#replayBuffer.sample(batchSize);
    const stateLength = this.totalInputLength;
    if (sampledReplays.nextState.findIndex(value => value == null || Number.isNaN(value)) !== -1)
      this.throwError('learn', `sampledReplays.nextStateに無効な値(NaN, null相当)が含まれています。nextState: ${sampledReplays.nextState.join(',')}`);
    // 次の状態における全てのQ値を取得
    const nextActions = this.#targetNetwork.forward({
      inputs: sampledReplays.nextState,
      batchSize
    });
    if (nextActions.findIndex(value => value == null || Number.isNaN(value)) !== -1)
      this.throwError('learn', `nextActionsに無効な値(NaN, null相当)が含まれています。nextActions: ${nextActions.join('\n')}`);
    
    const actions = this.#onlineNetwork.forward({
    ...
    let duList = new Float32Array(outputLength*batchSize).fill(0);
    
    // バッチ毎に計算
    for (; batchIdx < batchSize; batchIdx++) {
      // ===== TD誤差(損失)を求める =====
      reward = rewardList[batchIdx];
      tdTarget = reward + gamma * Math.max(...nextActions.subarray(outputOffset, outputOffset+outputLength));
...
class Network extends Common {
  ...
  #forwardJS({ inputs, batchSize }) {
    ...
    for (let layer of this.layers) {
      ...
      if (nextLayerInput.findIndex(value => value == null || Number.isNaN(value)) !== -1)
    ...
    switch (this.optimizerType) {
      ...
      case "adam":
        this.savedOptValues = {
          ...
          w_v: new Float32Array(weightLength),
          w_powBeta1: new Float32Array(weightLength),
          w_powBeta2: new Float32Array(weightLength),
          b_m: new Float32Array(biasLength),
          b_v: new Float32Array(biasLength),
          b_powBeta1: new Float32Array(biasLength),
          b_powBeta2: new Float32Array(biasLength),
        }
        break;
    }
  }
  ...
  backward(duList, batchSize) {
    ...
    */
    ...
    for (let batchIdx = 0; batchIdx < batchSize; batchIdx++) {
      ...
      // δ^l_j
      for (let outIdx = 0; outIdx < totalOut; outIdx++) {
        deltaList[outIdx] = primeFunc(savedValues[batchIdx * totalOut + outIdx])*duList[duListOffset+outIdx];
      }
      ...
      for (let outIdx = 0; outIdx < totalOut; outIdx++) {
        delta = deltaList[outIdx];
        b = biases[outIdx];
        switch (optType) {
          case "sgd":
            biasesOriginal[outIdx] = optFunc(b, delta)[0];
            break;
          case "momentum":
          case "rmsprop":
            [
          ...
          case "adam":
            [
              ...
              savedOptValues.b_v[outIdx],
              biasesOriginal[outIdx]
            ] = optFunc(b_powBeta1[outIdx], b_powBeta2[outIdx], b_m[outIdx], b_v[outIdx], b, delta);
            break;
        }
        if (Number.isNaN(biasesOriginal[outIdx]))
        ...
        for (let inIdx = 0; inIdx < totalIn; inIdx++) {
          ...
          z = inputs[batchIdx * totalIn + inIdx];
          w = weights[weightOffset];
          
          switch (optType) {
            case "sgd":
              weightsOriginal[weightOffset] = optFunc(w, delta*z)[0];
              break;
            case "momentum":
            case "rmsprop":
              [
              ...
              ] = optFunc(w_v[weightOffset], w, delta*z);
              break;
            case "adam":
              const res = optFunc(w_powBeta1[weightOffset], w_powBeta2[weightOffset], w_m[weightOffset], w_v[weightOffset], w, delta*z);
              
              savedOptValues.w_powBeta1[weightOffset] = res[0];
              savedOptValues.w_powBeta2[weightOffset] = res[1];
              savedOptValues.w_m[weightOffset] = res[2];
              savedOptValues.w_v[weightOffset] = res[3];
              weightsOriginal[weightOffset] = res[4];
              break;
          }
          if (weightsOriginal[weightOffset] == null || Number.isNaN(weightsOriginal[weightOffset]))
