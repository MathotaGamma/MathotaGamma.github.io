...
class Common {
  ...
  // ===== NaN/Infinity/null判定強化 (v2で追加) =====
  // null, undefined, NaN, +Infinity, -Infinity のいずれかを検出する。
  // Number.isNaN単体だとInfinityを見逃す(Infinity伝播→NaN化するまで気づけない)ため、
  // Number.isFiniteベースの一括判定に統一する。
  // 見つかったら最初に無効だったindexを返す。無ければ-1。
  hasInvalidValue(array, start=0, end=array.length) {
  ...
  // hasInvalidValueで見つけたindexから、周辺の値も含めた診断用メッセージを作る。
  // (巨大配列(画像入力等)を丸ごとjoinしてログ化するのを避けるため)
  invalidValueMessage(array, index) {
    const value = array[index];
    const around = typeof array.subarray === 'function'
      ? this.showArrayAroundIndex(array, index, 5)
      : String(value);
    return `index: ${index}, value: ${value}, 周辺: ${around}`;
  }
}
...
configure ${configureArgs}`
  ...
  step(state) {
    ...
    // v2: NaN/Infinity強化。evaluationStepと同様にactionsの値自体も検証する。
    const invalidActionIdx = this.hasInvalidValue(actions);
    if (invalidActionIdx !== -1)
      this.throwError('step', `#forwardの戻り値に無効な値(NaN/Infinity/null)が含まれています。${this.invalidValueMessage(actions, invalidActionIdx)}`);
  ...
  evaluationStep(state) {
    ...
    if (actions == null)
      this.throwError('evaluationStep', '#forwardの戻り値がnull相当です。');
    // v2: includes(NaN)はInfinityを見逃すため、null/NaN/Infinityを一括判定するhasInvalidValueに統一。
    const invalidIdx = this.hasInvalidValue(actions);
    if (invalidIdx !== -1)
      this.throwError('evaluationStep', `#forwardの戻り値に無効な値(NaN/Infinity/null)が含まれています。${this.invalidValueMessage(actions, invalidIdx)}`);
  ...
  })
  {
    ...
    // v2: NaNのみだとInfinityを見逃すため、hasInvalidValueに統一(null/NaN/±Infinityを検出)。
    if (this.isCheck) {
      const invalidIdx = this.hasInvalidValue(ret);
      if (invalidIdx !== -1)
        this.throwError('#forward', `戻り値に無効な値が含まれています。${this.invalidValueMessage(ret, invalidIdx)}`);
    }
    return ret;
  }
  ...
  })
  {
    ...
    // v2: hasInvalidValueに統一(null/NaN/±Infinityを検出)。
    if (this.isCheck) {
      const invalidIdx = this.hasInvalidValue(ret);
      if (invalidIdx !== -1)
        this.throwError('#forward', `戻り値に無効な値が含まれています。${this.invalidValueMessage(ret, invalidIdx)}`);
  ...
  */
  learn(args=null) {
    ...
    // v2: 巨大配列(画像入力等)を丸ごとjoinしてエラーメッセージ化するとログが膨大になるため、
    // invalidValueMessage(周辺のみ表示)に変更。NaN専用判定もhasInvalidValueへ統一(Infinityも検出)。
    if (this.isCheck) {
      const invalidIdx = this.hasInvalidValue(sampledReplays.nextState);
      if (invalidIdx !== -1)
        this.throwError('learn', `sampledReplays.nextStateに無効な値(NaN, Infinity, null相当)が含まれています。${this.invalidValueMessage(sampledReplays.nextState, invalidIdx)}`);
    }
    // 次の状態における全てのQ値を取得
    ...
    if (modelType === "dqn") {
      ...
      if (this.isCheck) {
        const invalidIdx = this.hasInvalidValue(nextActions);
        if (invalidIdx !== -1)
          this.throwError('learn', `nextActionsに無効な値(NaN, Infinity, null相当)が含まれています。${this.invalidValueMessage(nextActions, invalidIdx)}`);
      }
    }
      ...
      // v2: ddqnのonlineArgsContainer(targetForward結果)も同様に検証する。
      // (旧実装ではここが未検証で、全出力NaN化→maxIdx=-1のまま負インデックス参照、
      //  というクラッシュ経路の温床になっていた)
      if (this.isCheck) {
        const invalidIdx = this.hasInvalidValue(onlineArgsContainer);
        if (invalidIdx !== -1)
          this.throwError('learn', `onlineArgsContainerに無効な値(NaN, Infinity, null相当)が含まれています。${this.invalidValueMessage(onlineArgsContainer, invalidIdx)}`);
      }
    }
    ...
    for (; batchIdx < batchSize; batchIdx++) {
      ...
      if (modelType === "dqn")
        ...
        // v2: 全出力がNaN等で maxVal<val が一度も成立しない場合、maxIdxが-1のまま
        // (=負インデックスで別要素を誤って読む/初期batchでは配列外アクセスになる)クラッシュ経路を明示的に検知する。
        if (maxIdx === -1)
          this.throwError('learn', `nextActionsの中で有効な最大値が見つかりませんでした(全出力がNaN/Infinityの可能性)。batchIdx: ${batchIdx}, outputOffset: ${outputOffset}, nextActions(該当区間): ${Array.from(nextActions.subarray(outputOffset, outputOffset+outputLength)).join(',')}`);
        expectedValue = onlineArgsContainer[outputOffset+maxIdx];
      }
        
      tdTarget = reward + (1-doneList[batchIdx]) * gamma * expectedValue;
      
      // v2: NaNのみの判定だとInfinityを見逃す(expectedValueがInfinityの場合tdTargetもInfinityになり得るが
      // isNaN(Infinity)===falseのためすり抜けていた)ため、!Number.isFinite()に強化。
      if (tdTarget == null || !Number.isFinite(tdTarget))
        this.throwError('learn', `tdTargetが無効な値です(NaN, Infinity, null相当)。reward: ${reward}, gamma: ${gamma}, expectedValue: ${expectedValue}, done: ${doneList[batchIdx]}`);
      ...
      // v2: 同様にInfinityも検知するよう強化。
      if (tdErrorPrime == null || !Number.isFinite(tdErrorPrime))
        this.throwError('learn', `tdErrorPrimeが無効な値です(NaN, Infinity, null相当)。tdTarget: ${tdTarget}, q: ${q}, action: ${action}`);
...
class Network extends Common {
  ...
  #forwardJS({ inputs, batchSize }) {
    ...
    for (let layer of this.layers) {
      ...
      // v2: hasInvalidValueに統一(null/NaN/±Infinityを検出)。
      if (isCheck) {
        const invalidIdx = this.hasInvalidValue(nextLayerInput);
        if (invalidIdx !== -1)
          this.throwError('#forwardJS', `nextLayerInputに無効な値が含まれています。kind: ${layer.getKind()}, info: ${JSON.stringify(layer.getInfo())}, ${this.invalidValueMessage(nextLayerInput, invalidIdx)}`);
      }
    }
...
class Layer extends Common {
  ...
  inputCheck(inputs, batchSize) {
    ...
    // v2: includes(null)+includes(NaN)の2パス走査(かつInfinityを見逃す)から、
    // 1パスでnull/NaN/±Infinityをまとめて検出するhasInvalidValueに統一。
    const invalidIdx = this.hasInvalidValue(inputs);
    if (invalidIdx !== -1)
      this.throwError('forward', `入力に不適当な値(NaN/Infinity/null)が入っています。${this.invalidValueMessage(inputs, invalidIdx)}`);
  }
...
class DenseLayer extends Layer {
  ...
  forward(inputs, batchSize) {
    ...
    if (this.isCheck) {
      // v2: findIndexでのNaN専用判定(Infinityを見逃す)から、hasInvalidValueに統一。
      let index;
      index = this.hasInvalidValue(biases);
      if (index !== -1)
        this.throwError('forward', `check...biasesに無効な値が含まれています。units: ${this.units}, biases: ${this.showArrayAroundIndex(biases, index, 5)}`);
      index = this.hasInvalidValue(weights);
      if (index !== -1)
  ...
  backward(duList, batchSize) {
    ...
    */
    ...
    if (isCheck) {
      // v2: hasInvalidValueに統一(null/NaN/±Infinityを検出)。
      const index = this.hasInvalidValue(duList);
      if (index !== -1)
        this.throwError('backward', `duListに無効な値が含まれています(NaN, Infinity, null相当)。units: ${this.units}, duList[${index}付近]: ${this.showArrayAroundIndex(duList, index, 5)}`);
    }
    ...
    for (let batchIdx = 0; batchIdx < batchSize; batchIdx++) {
      ...
      for (let inIdx = 0; inIdx < totalIn; inIdx++) {
        ...
        // v2: NaNのみだとInfinityを見逃すため、!Number.isFiniteに強化。
        if (isCheck && (sum == null || !Number.isFinite(sum)))
          this.throwError('backward', `retDuListに設定する値が無効です(NaN, Infinity, null相当)。batchIdx: ${batchIdx}, inIdx: ${inIdx}, sum: ${sum}`);
    ...
    if (maxNorm != null && maxNorm > 0) {
      ...
      // v2: sumSq自体がInfinity(既にdW/dBが発散していた場合)になっているとtotalNormもInfinityになり、
      // scaleが0や不定になりうる。isCheck時はここで早期検知する。
      if (isCheck && !Number.isFinite(totalNorm))
        this.throwError('backward', `勾配のL2ノルムが無効な値です(NaN, Infinity)。totalNorm: ${totalNorm}, sumSq: ${sumSq}`);
    }
    else {
      // クリッピングがオフの場合
      for (let outIdx = 0; outIdx < totalOut; outIdx++) {
    ...
    for (let outIdx = 0; outIdx < totalOut; outIdx++) {
      ...
      // v2: Number.isNaN単体からNumber.isFiniteベースに強化(Infinityも検知)。
      if (isCheck && !Number.isFinite(biasesOriginal[outIdx]))
        this.throwError('backward',`biasで数値エラーになりました。units=${this.units}, outIdx=${outIdx}, b=${b}, gradB=${gradB}, delta=${delta}`)
      ...
      for (let inIdx = 0; inIdx < totalIn; inIdx++) {
        ...
        // v2: 【重要バグ修正】元は
        //   if (isCheck && weightsOriginal[...] == null || Number.isNaN(weightsOriginal[...]))
        // となっており、&&が||より優先されるため
        //   (isCheck && (... == null)) || Number.isNaN(...)
        // と解釈され、Number.isNaN側の判定がisCheckに関係なく常に実行されていた。
        // 括弧で明示し、かつInfinityも検知するよう強化。
        if (isCheck && (weightsOriginal[weightOffset] == null || !Number.isFinite(weightsOriginal[weightOffset])))
          this.throwError('backward',`weightで数値エラーになりました。units=${this.units}, weightOffset=${weightOffset}, w=${w}, gradW=${gradW}, optType=${optType}`)
      }
    }
    
    // console.log('DenseLayerのbackwardに要した時間 : '+String((Date.now()-startTime)/1000)+' s');
    // v2: 元は常時実行(isCheck無視)でNaNのみ判定していた。isCheck時のみに揃え、Infinityも検知するよう強化。
    // (isCheckがfalseの通常運用時は、上のper-weightチェックで既に弾かれているため二重コストを避ける)
    if (isCheck) {
      const invalidIdx = this.hasInvalidValue(this.weights);
      if (invalidIdx !== -1)
        this.throwError('backward', `backward後のweightsに無効な値が含まれています。${this.invalidValueMessage(this.weights, invalidIdx)}`);
    }
    return retDuList;
  }
}
